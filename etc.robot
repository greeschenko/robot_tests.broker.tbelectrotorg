...
Library     Collections
Library     etc_helper.py


*** Variables ***
${TESTDOMAIN}                                        http://test.etc-torgi.com
${sign_in}                                           id=loginbtn
...
Задати запитання на тендер
  ...
  etc.Задати питання  ${username}  ${tender_uaid}  ${question}
...
Отримати інформацію із тендера
    ...
    ${return_value}=  Run Keyword If
    ...  'status' in '${field_name}'       convert_etc_string    ${return_value}
...
Отримати інформацію із документа
  ...
  ${tender}=  etc.Пошук тендера по ідентифікатору  ${username}  ${tender_uaid}
...
Завантажити фінансову ліцензію
    ...
    etc.Завантажити документ в ставку  ${username}  ${filepath}  ${tender_uaid}
...
Отримати інформацію із пропозиції
    ...
    ${bid}=   etc.Отримати пропозицію  ${field}
...
Отримати інформацію із об'єкта МП
    ...
    ${return_value}=  Run Keyword If
    ...  'status' in '${fieldname}'                                   convert_etc_string  ${return_value}
    ...  ELSE IF    'registrationDetails.status' in '${fieldname}'    convert_etc_string  ${return_value}
    ...  ELSE IF    'rectificationPeriod.endDate' in '${fieldname}'  convert_etc_date_to_iso_format  ${return_value}
...
Створити лот
    ...
    etc.Пошук об’єкта МП по ідентифікатору  ${username}  ${asset_uaid}
...
Отримати інформацію із лоту
    ...
    ...  ELSE IF    'registrationDetails.status' in '${fieldname}'    convert_etc_lot_string  ${return_value}
    ...  ELSE IF    'procurementMethodType' in '${fieldname}'    convert_etc_lot_auction_string  ${return_value}
    ...  ELSE IF    'rectificationPeriod.endDate' in '${fieldname}'  convert_etc_date_to_iso_format_with_tz  ${return_value}
    ...  ELSE IF    'auctionPeriod' in '${fieldname}'  convert_etc_date_to_iso_format  ${return_value}
    ...  ELSE IF    'quantity' in '${fieldname}'  Convert To Number  ${return_value}
...
Отримати інформацію з активу лоту
    ...
    ...  'status' in '${fieldname}'                                   convert_etc_lot_string  ${return_value}
    ...  ELSE IF    'registrationDetails.status' in '${fieldname}'    convert_etc_lot_string  ${return_value}
    ...  ELSE IF    'procurementMethodType' in '${fieldname}'    convert_etc_lot_string  ${return_value}
    ...  ELSE IF    'rectificationPeriod.endDate' in '${fieldname}'  convert_etc_date_to_iso_format_with_tz  ${return_value}
    ...  ELSE IF    'auctionPeriod' in '${fieldname}'  convert_etc_date_to_iso_format  ${return_value}
    ...  ELSE IF    'quantity' in '${fieldname}'  Convert To Number  ${return_value}
...
Додати умови проведення аукціону для індексу 0
    ...
    ${minimalStep.valueAddedTaxIncluded}=   convert_etc_string       ${minimalStep.valueAddedTaxIncluded}
    ...
    ${auctionPeriod.startDate}=             etc_convertdate     ${auctionPeriod.startDate}
